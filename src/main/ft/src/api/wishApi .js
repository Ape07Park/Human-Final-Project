...
export const fetchWishList = async (userInfo) => {
  try {
    const response = await apiClient.post('/ft/wish/list', { email: userInfo.email }); // 이메일 데이터 보내기
...
export const handleLikeClickAPI = async (iid, userEmail) => {
  try {
    const response = await apiClient.post(`/ft/wish/click`, {
      iid: iid,
      email: userEmail
    });
    return response;
...
export const fetchItemWishCounts = async (iid) => {
  try {
    const response = await apiClient.get(`/ft/wish/count/${iid}`);
    return response;
  } catch (error) {
